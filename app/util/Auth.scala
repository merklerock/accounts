...
object Auth {
  ...
  def Authenticated(username:String, sessionId:String)(f: AuthenticatedRequest => Result) = {
     import srp._     
	 Action { request =>
	    if(MerklerRockSRPServer.validSession(username, sessionId))
  ...
  def EncryptedAuthenticatedPage(username:String, sessionId:String, page:String)(f :Tuple5[String,String,String,Request[AnyContent], String => String] => Result) = {
	 ...
	 Action { request =>
	    val (s,h) = MerklerRockSRPServer.getSessionWithHash(username)
	    ...
	    if(MerklerRockSRPServer.validSession(username, new String(sessionIdDec, UTF8))){
